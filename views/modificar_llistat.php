...
<body>
    ...
    <input type="hidden" name=controlador value=Llistat>
        ...
        <table>
            <?php 
                ...
                foreach($item as $key=>$value) { 
                    if($key == "id_llibre"){ ?>
                        <tr>
                            <td><label for=id_llibre>id_llibre </label></td>
                            <td><SELECT id=id_llibre name=id_llibre>
                                <option value=0></option> <?php
                                while($class_reg = $classes->fetch()){ ?>
                                    <option value=<?php echo $class_reg["id_llibre"] ?>
                                    <?php echo ($class_reg["id_llibre"]==$value ?" selected":"") ?>>
                                        <?php echo $class_reg["id_llibre"] ?>
                                    </option><?php
