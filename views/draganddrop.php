...
function carregar_classificacio() {
  var xhr = new XMLHttpRequest();
  xhr.onreadystatechange = function(){
    if(this.readyState==4) {
      dades = JSON.parse(this.responseText);
      dades.foreach(dada => {
       document.getElementById("box_"+dada.id_box).appendChild(document.getElementById("item_"+dada.id_item));
      });
  }
  };
  xhr.open("GET","index.php?controlador=Classif&accion=carregar_classificacio",true);
...
<body>
  <input type="button" onclick="carregar_classificacio()" value="Carregar Classificacio">

<div class=base_treballadors>
    <?php
  while($treballador = $treballadors->fetch()) { ?>
      <div class="treballadors"  id_treballador=<?= $treballador["id_treballador"]?> id="treballador_<?= $treballador["id_treballador"] ?>"  draggable="true" ondragstart="drag(event)" >   <?= $treballador["nom"] ?> </div><?php
  } ?>
</div>
